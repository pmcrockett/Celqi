...
### MAIN MENU
##### FILE
* **New map ...** — Closes current map and creates a new map. You will be prompted to save the current map if it has changed.
* **Save map** — Saves map under its current name.
* **Save map as ...** — Saves map under a different name.
* **Load map ...** — Closes current map and opens a saved map. You will be prompted to save the current map if it has changed.
...
##### BRUSH PROPERTIES
* **Object color ...** — Sets the color of the brush object. For primitives, `{1, 1, 1, 1}` represents white, while for models, it represents the original model texture. 
...
##### ROTATION OPTIONS
* **Snap mode** — Changes if and how an object snaps to the grid when rotated. *Snap by* changes the rotation by the given value. *Snap to* snaps the rotation to a multiple of the given value. Objects always rotate around their position origin.
...
Some of Celqi's functions ask for an object's rotation as represented by foward and up vectors. If you track objects' rotations in your own code using a different method, you will need to convert the rotations using either `eulerToDirVec()` or `axisAngleToDirVec()`, both of which return a struct in the form `[ .fwd = VECTOR, .up = VECTOR ]`.
...
Celqi has a built-in physics engine; to see a physics demo, choose *Advanced > Throw physics object* from the main menu or press **(A)** while in photo mode. Thrown objects will disappear after several seconds. While in photo mode, objects can be immediately cleared with **(B)** or boosted into the air by holding **(Y)**.
...
|opt|`_getAllCollisions`|bool|`false`|Whether to return only the first collision found by the check (`false`) or all of the collisions found by the check (`true`). Returning all of the collisions is slower.|
...
|opt|`_grav`|vector3|`{0, -1, 0}`|Specifies the direction and strength of gravity. `{0, -50, 0}` approximates Earth gravity.|
...
|opt|`_initAuxCollider`|bool|`true`|Whether to also initialize an auxilliary collider, which is used to ensure that a fast-moving object will not clip through obstacles between collision checks.|
...
|opt|`_getAllCollisions`|bool|`false`|Whether to return all collisions rather than just the first collision found.|
|opt|`_getNormal`|bool|`false`|Whether to calculate the direction of faces that the object collides with. Necessary for redirecting velocity along a surface instead of stopping short.|
|opt|`_affectRot`|bool|`true`|Whether collisions can impart rotational velocity to the object.|
|opt|`_maintainOrthoR`|bool|`false`|Whether the object's rotational velocity should be set in a way that keeps the object's local right vector othogonal with the world space up vector.|
...
|opt|`_useGrav`|bool|`false`|Whether velocity is affected by gravity.|
...
|`.colPos`|vector3|Object position during most recent collision.|
|~~`.colScale`~~|~~vector3~~||
|~~`.colThisFrame`~~|~~bool~~||
|~~`.colUp`~~|~~vector3~~||
|~~`.comPos`~~|~~vector3~~||
...
|`.scale`|vecotr3|Scale of the object after the most recent update.|
