...
function coreLoader(ref _in, _funcName, _argArr)
	loop if _funcName == "isEditor" then
		_in = false
		break endif
	if _funcName == "loadMapMsg" then
		_in = loadMapMsg(_argArr[0], _argArr[1], _argArr[2])
		break endif
	if _funcName == "loadDefMsg" then
		_in = loadDefMsg(_argArr[0])
		break
	endif break repeat
return _in
*/
...
function readMap(_file, _mapName)
	...
	function readMap_addQueued(_queued, _queuedLight, _mapName)
					...
					if !isDelDef then
						...
						if decodeBank(-1, newObj.bankIdx) >= 0 and decodeIdx(-1, newObj.bankIdx) >= 0 then
							if len(queuedObj) then
								placementTimer = time()
								addResult = readMap_addQueued(queuedObj, queuedLight, _mapName)
...
// Checks if _item is present in _arr.
...
// Normalizes only if the result will not be {nan, nan, nan}.
...
// Returns the actual mapObj that a reference struct points to.
function getMapObjFromStruct(_cellIdxStruct)
return getMapObjFromRef(g_cell[_cellIdxStruct.cell][_cellIdxStruct.idx])

function getMapObjFromStruct(_cell, _idx)
return getMapObjFromRef(g_cell[_cell][_idx])
...
function decodeIdx(_cellIdx)
return bitFieldExtract(_cellIdx, 0, 16)
...
function getMapObjName(_mapObj)
return getMapObjName("", _mapObj)

function getMapObjName(ref _in, _mapObj)
	var isRef
	isMapObjRef(isRef, _mapObj)
	
	if isRef then
		_mapObj = getMapObjFromRef(_mapObj)
	endif
	...
	// During merged def loads, .bankIdx may temporarily contain a missing object's
	// name, so we screen this with len().
	var bankIdxLen = len(_mapObj.bankIdx)
	var bankIdxStr = str(_mapObj.bankIdx) // Avoid multiple nested ifs to reduce stack size
	
	if !bankIdxLen and bankIdxStr != "-1" then
		var bank
		decodeBank(bank, _mapObj.bankIdx)
		var idx
		decodeIdx(idx, _mapObj.bankIdx)
		var objBank = g_obj[bank]
...
// Initializes a colContext for use with getSimpleMapCollision().
function initSimpleColContext(ref _colCon, _obj, _pos, _fwd, _up, _scale, _collisionMode)
	_colCon = initColContext(_colCon, _obj, _pos, _fwd, _up, _scale, _collisionMode, 1, {0, 0, 0}, 0.001, false, true, true)
return _colCon
...
function getSimpleMapCollision(ref _colCon, _obj, _pos, _fwd, _up, _scale, _getAllCollisions, _forceUpdate)
return getSimpleMapCollision(_colCon, _obj, _pos, _fwd, _up, _scale, _getAllCollisions, _forceUpdate, _colCon.collisionMode)

function getSimpleMapCollision(ref _colCon, _obj, _pos, _fwd, _up, _scale, _getAllCollisions, _forceUpdate, _collisionMode)
	var cellResult = updateCellContext(_colCon.cell, _colCon.cellPos, _pos, false, true)
	
	if _forceUpdate then
		_colCon.objList = []
	endif
	
	updateColContext(_colCon, cellResult.cell, _pos, _fwd, _up, _colCon.ext, _scale, _obj, 
		_getAllCollisions, _collisionMode, _colCon.comPos, false, false, [], [])
...
function initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes)
return initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes, true, true, true)

function initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes, _initAuxColllider)
return initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes, _initAuxColllider, true, true)

function initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes, _initAuxColllider, _initExt)
return initColContext(_colCon, _obj, _comPos, _fwd, _up, _scale, _mode, _mass, _grav, _extRes, _initAuxColllider, _initExt, true)
...
// Add velocity to a colContext and unfreezes the object.
function addColContextVel(ref _colCon, _vel)
	_colCon.vel += _vel
	_colCon.frozen = 0
return _colCon

// Sets the velocity of a colContext and unfreezes the object.
function setColContextVel(ref _colCon, _vel)
	_colCon.vel = _vel
	_colCon.frozen = 0
return _colCon

// This is the main collision check function. When given an object, it calculates whether placing the object 
...
return updateObjPhysics(_obj, _colCon, _scale, _collisionMode, _forceUpdate, _getAllCollisions, _getNormal, _affectRot, 
		_maintainOrthoR, _normRecurLimit, _useGrav, deltaTime(), {0, 0, 0})

function updateObjPhysics(_obj, ref _colCon, _scale, _collisionMode, _forceUpdate, _getAllCollisions, _getNormal,
		_affectRot, _maintainOrthoR, _normRecurLimit, _useGrav)
return updateObjPhysics(_obj, _colCon, _scale, _collisionMode, _forceUpdate, _getAllCollisions, _getNormal, _affectRot, 
		_maintainOrthoR, _normRecurLimit, _useGrav, _delta, {0, 0, 0})

function updateObjPhysics(_obj, ref _colCon, _scale, _collisionMode, _forceUpdate, _getAllCollisions, _getNormal, 
		_affectRot, _maintainOrthoR, _normRecurLimit, _useGrav, _delta, _addVel)
	...
	// As object freezes, reduce the distance it moves in a tick. This helps low-framerate objects get loose
	// from situations that would freeze them in the air because they're unable to calculate viable collision
	// resolutions due to their high movement deltas
	if _colCon.frozen and _normRecurLimit > 0 then
		_delta = _delta * (1 - _colCon.frozen / g_physicsFreezeThreshold)
	endif
	
	var fwd = getNewColFwd(_colCon, _delta)
	var up
	
	// _maintainOrthoR ensures that the object's right vector will stay orthogonal with worldspace up.
	// In practical terms, this means it will maintain a rotation like you would get from objectPointAt() 
	// or like would be appropriate for a camera that cannot change its roll value.
	if !_maintainOrthoR then
		up = getNewColUp(_colCon, _delta)
	else
		var r = safeCross(fwd, {0, 1, 0})
		
		if r == {0, 0, 0} then
			up = getNewColUp(_colCon, _delta)
		else
			up = axisRotVecBy(r, fwd, -90)
		endif
	endif
	...
	if _colCon.frozen >= g_physicsFreezeThreshold then
		...
		if _useGrav and _normRecurLimit > 0 then
			...
			// If moving fast enough to cause problems, scale velocity down
			if length((_colCon.vel + _colCon.grav * _delta) * _delta) > 7 then
				var scaleFactor = 7 / length((_colCon.vel + _colCon.grav * _delta) * _delta)
				var newGrav = _colCon.grav * scaleFactor
				var newVel = _colCon.vel * scaleFactor
				
				newPosDelta = (newVel + newGrav * _delta) * _delta
				_colCon.vel = _colCon.vel + (_colCon.grav * _delta)
			else
				newPosDelta = (_colCon.vel + _colCon.grav * _delta) * _delta
				_colCon.vel = _colCon.vel + (_colCon.grav * _delta)
			endif
			
			comPos = _colCon.comPos + newPosDelta
		endif
		
		prevOrig = getOriginFromCom(prevComPos, _colCon.centerOfMass, prevFwd, prevUp, prevScale)
		orig = getOriginFromCom(comPos, _colCon.centerOfMass, fwd, up, _scale)
	...
	if checkCol then
		...
		if fwd != prevFwd or up != prevUp then
			setObjRot(_obj, orig, fwd, up)
		endif
		...
		center = changeVecSpaceZFwd(center, fwd, up, safeCross(fwd, up), {0, 0, 0})
		...
		var colBuf = _colCon.colThisFrame
		var rotVelBuf = _colCon.rotVel
		var rotVelAxisBuf = _colCon.rotVelAxis
		...
		if _collisionMode <= 1 or distance(prevOrig + prevCenter, orig + center) <= objMinDim then
			...
			updateColContext(_colCon, cellResult.cell, orig, fwd, up, 
				_colCon.ext, _scale, _obj, _getAllCollisions, _collisionMode, prevOrig, _getNormal, _affectRot, [], [])
			...
			var lineScale = {objMaxDim, objMaxDim, distance(prevOrig + prevCenter, orig + center) / 2 + length(lineDir * objMaxDim)}
			...
			updateColContext(auxCon, cellResult.cell, linePos, lineDir, lineUp, 
				auxCon.ext, lineScale, _colCon.auxCollider, true, auxCon.collisionMode, prevOrig + prevCenter, false, false, [], [])
			_colCon.auxColCon = auxCon
			
			cellResult = updateCellContext(_colCon.cell, _colCon.cellPos, orig, false, _forceUpdate)
			updateColContext(_colCon, cellResult.cell, orig, fwd, up, 
				_colCon.ext, _scale, _obj, _getAllCollisions, _collisionMode, prevOrig, _getNormal, _affectRot, 
				auxCon.collision, auxCon.gridBit)
		...
		if colBuf then
			_colCon.rotVel = rotVelBuf
			_colCon.rotVelAxis = rotVelAxisBuf
		endif
		...
		if _colCon.colThisFrame then
			...
			// Cap delta for deflection to ensure the next recursive collision check will happen in a 
			// reasonable position.
			if _delta >= 0.064 then
				deflDelta = 0.064
			else
				deflDelta = _delta
			endif
			...
			if len(_colCon.normal) and _getNormal and _normRecurLimit then
				...
				if len(_colCon.colDepth) then
					...
					if _colCon.colDepth[0] < float_max and _colCon.normal[0] != {0, 0, 0} then
						// If rotation is affected, bounce off the collision point
						if _affectRot then
							deflPos = comPos + safeNormalize(prevComPos - comPos) * _colCon.colDepth[0]
						// If rotation isn't affected, extrapolate position by sliding along surface
						else
							deflPos = comPos + _colCon.normal[0] * _colCon.normColDepth[0]
						endif
				...
				if _affectRot then
					...
					_colCon.vel = safeNormalize(reflect(_colCon.vel, _colCon.normal[0]))
					_colCon.rotVel = 0
				endif
				
				if _useGrav then
					if _colCon.rotVelAxis != {0, 0, 0} and _affectRot then
						var angle = min(_colCon.rotVel / (_colCon.mass * 4), 90)
						_colCon.vel = axisRotVecBy(_colCon.vel, _colCon.rotVelAxis, _colCon.rotVel / (_colCon.mass * 4))
					endif
					
					var newPosDelta
					// If moving fast enough to cause problems, scale velocity down
					if length((_colCon.vel + _colCon.grav * deflDelta) * deflDelta) > 7 then
						var scaleFactor = 7 / length((_colCon.vel + _colCon.grav * deflDelta) * deflDelta)
						var newGrav = _colCon.grav * scaleFactor
						var newVel = _colCon.vel * scaleFactor
						newPosDelta = (newVel + newGrav * deflDelta) * deflDelta * scaleFactor
					else
						newPosDelta = (_colCon.vel + _colCon.grav * deflDelta) * deflDelta
					endif
					
					deflPos += newPosDelta
					
					if _normRecurLimit > 0 and len(_colCon.normal) and _affectRot then
						if _colCon.normal[0] != {0, 0, 0} then
							var r = safeCross(fwd, up)
							
							var limit = 0.5
							if (_colCon.normAxis == "fwd" and abs(dot(fwd, _colCon.normal[0])) < limit) 
									or (_colCon.normAxis == "up" and abs(dot(up, _colCon.normal[0])) < limit) 
									or (r != {0, 0, 0} and _colCon.normAxis == "r" and abs(dot(r, _colCon.normal[0])) < limit) 
									or length(_colCon.vel) > length(_colCon.grav) * 4 then
								resetFwdUp = false
							else
								_colCon.rotVel *= clamp(0.016 / _delta, 0, 0.75)
								
								var normAxis
								
								if _colCon.normAxis == "fwd" and abs(dot(fwd, _colCon.normal[0])) >= limit then
									normAxis = fwd * getSign(dot(fwd, _colCon.normal[0]))
								else if _colCon.normAxis == "up" and abs(dot(up, _colCon.normal[0])) >= limit then
									normAxis = up * getSign(dot(up, _colCon.normal[0]))
								else
								...
								if axis != {0, 0, 0}  then
									if deg < 1 then
										_colCon.rotVel = 0
										fwd = axisRotVecBy(fwd, axis, deg)
										up = axisRotVecBy(up, axis, deg)
									else
										var dirResult = lerpDirVecs(fwd, up, axisRotVecBy(fwd, axis, deg), 
											axisRotVecBy(up, axis, deg), clamp(_delta * 10, 0.25, 1))
										fwd = dirResult.fwd
										up = dirResult.up
									endif
									
									// Prevent object from coming to rest on slopes by adding a bit of velocty to kick it away from
									// surfaces that don't directly oppose gravity.
									if dot(_colCon.normal[0], safeNormalize(_colCon.grav)) > -0.9 then
										_colCon.vel += _colCon.normal[0] * 1
									endif
								endif
							endif
						endif
				...
				var oldPos = argColCon.comPos
				
				updateObjPhysics(_obj, argColCon, _scale, _collisionMode, _forceUpdate, 
					_getAllCollisions, _getNormal, _affectRot, _maintainOrthoR, 
					abs(_normRecurLimit) * -1 + 1, false, deflDelta, deflPos)
				_colCon = argColCon
		...
		if _normRecurLimit >= 0 then
			if _colCon.comPos == prevComPos
					and _colCon.fwd == prevFwd
					and _colCon.up == prevUp
					and _colCon.scale == prevScale then
				_colCon.frozen += 1
			else
				_colCon.frozen = 0
			endif
...
// _collisionMode can be 1 (bounding box), 2 (single raycast), 3 (spheroid raycasts), 
// or 4 (cuboid raycasts).
function updateColContext(ref _colCon, _cell, _pos, _fwd, _up, _ext, _scale, _collider, _getAllCollisions, _collisionMode, _prevPos, _getNormal, _affectRot, _colOverride, _gridBitOverride)
	var timer = time()
	_colCon.collision = []
	_colCon.normal = []
	_colCon.colDepth = []
	_colCon.normColDepth = []
	...
	if _collisionMode >= 3 and len(_colCon.collision) then
		resolveComplexRaycastCollision(_colCon, _cell, _pos, _fwd, _up, _ext, _scale, _getAllCollisions, 
			_collisionMode, _prevPos, _getNormal, _affectRot, _gridBitOverride)
...
function resolveSimpleRaycastCollision(ref _colCon, _pos, _fwd, _up, _ext, _scale, _prevPos)
	...
	if length(_colCon.delta) == 0 then
		dir = _fwd
	else
		dir = pointResult.dir
	endif
	...
	for i = 0 to len(colPool) loop
		...
		if cast <= maxDim then
			...
			_colCon.normColDepth = [ float_max ]
...
function resolveComplexRaycastCollision(ref _colCon, _cell, _pos, _fwd, _up, _ext, _scale, _getAllCollisions, 
	...
	if closestCol.cast < float_max then
		castHit = closestCol.castDat[0] + closestCol.cast * closestCol.castDat[1]
		_colCon.colPos = castHit
	endif
	...
	if _getNormal and closestCol.cast < float_max then
		...
		if normCast >= float_max then
			...
			_colCon.normColDepth = push(_colCon.normColDepth, float_max)
		else
			...
			var depthStart
			var prevDepthStart
			if _colCon.colDat.invCastDir or closestCol.castDatIdx >= 7 then
				depthStart = closestCol.castDat[0] + closestCol.castDat[1] * closestCol.castDat[2]
				prevDepthStart = prevCastDat[closestCol.castDatIdx][0] + prevCastDat[closestCol.castDatIdx][1] 
					* prevCastDat[closestCol.castDatIdx][2]
			else
				depthStart = closestCol.castDat[0]
				prevDepthStart = prevCastDat[closestCol.castDatIdx][0]
			endif
			
			var depthProjNorm = projectVecToPlane(depthStart, normCastHit, norm)
			
			var angle = getAngleBetweenVecs(safeNormalize(depthProjNorm - depthStart), 
				safeNormalize(prevDepthStart - depthStart))
			
			var depthDistNorm = distance(depthStart, depthProjNorm)
			var hyp
			// Angle should never be above 90 (i.e. cos shouldn't be < 0) -- this means the collider started 
			// below the collision plane, which can happen at corners -- so if it is, use distance from the 
			// plane surface straight down to the collision point as a fallback for collision depth
			if cos(angle) > 0.25 then
				hyp = depthDistNorm / abs(cos(angle))
			else
				hyp = distance(depthStart, depthProjNorm)
			endif
			
			var overage = 0.01 // Add a small amount to ensure the depth clears the collision surface
			var pointDepth = hyp + overage
			_colCon.colDepth = push(_colCon.colDepth, pointDepth)
			_colCon.normColDepth = push(_colCon.normColDepth, depthDistNorm + overage)
		endif
...
// to its current up direction. Typically, the output of this function should be passed to updateObjPhysics()'s 
			...
			for x = 0 to len(xW) loop
				...
				for i = 0 to yW[y] loop
					for j = 0 to zW[z] loop
						...
						// (firstBitPos % cellW2) -- find y position disregarding z position 
						// floor((firstBitPos % cellW2) / cellW) -- express y position as a height from 0 to 5
						// -cellW * floor((firstBitPos % cellW2) / cellW) * clamp(y, 0, 1) -- discard entirely (* 0) if y is 0, 
						// 	that is, if we're in the base cell, or express the height offset as negating the starting position
						// 	if y is > 0, that is, if we're in a new cell
						// -cellW * floor((firstBitPos % cellW2) / cellW) * clamp(y, 0, 1) + i * cellW -- add in the amount of 
						// 	height units present in the cell being considered
						
						yOffset = -cellW * floor((firstBitPos % cellW2) / cellW) * clamp(y, 0, 1) + i * cellW
						
						zOffset = -1 * cellW2 * floor((firstBitPos % cellW3) / cellW2) * clamp(z, 0, 1) + j * cellW2
...
// ----------------------------------------------------------------
// IF RUNNING THE CORE LOADER, ALL CODE BELOW THIS POINT CAN BE
...
// Audio
var g_sndQueue = initQueue("Empty", 0, [ [ .beatPos = {0, 0}, .bpm = 120, .timeSig = 4 ] ]) // Each channel has a separate queue
var g_chMenu = 0
var g_chCursor = 3
var g_chWidget = 6
...
// Info about a sound command to be executed in the future
struct queuedSound
	float pos = 0
	int ch = 0
	str func = ""
...
function coreLoader(ref _in, _funcName, _argArr)
	...
	loop if _funcName == "isEditor" then
		_in = true
		break endif
	if _funcName == "setGrpLightsSprScale" then
	...
	if _funcName == "getGrpLightsTypes" then
		...
		break endif
	if _funcName == "loadMapMsg" then
		_in = loadMapMsgEd(_argArr[0], _argArr[1], _argArr[2])
		break endif
	if _funcName == "loadDefMsg" then
		_in = loadDefMsgEd(_argArr[0])
		break
	endif break repeat
return _in
...
function initCur()
	...
	g_cur.lastColContext = initSimpleColContext(g_cur.lastColContext, g_cur.collider, 
...
// It is best practice to trigger a map load from as shallow an execution context 
// as possible to avoid hitting FUZE's recursion and/or memory limits.
...
// Discards all elements before element at _idx.
...
// What are the world unit dimensions shown on screen at a given distance
function getScreenRange(_dist, _fov)
	var scrY = _dist * tan(_fov / 2) * 2
	var scrX = scrY * (16 / 9)
return {scrX, scrY}
	...
	while !c.a or g_cDat[g_cIdx.a].held == true loop
		...
		if menuDirInputStarted(c, kbResult.kbActive) then
			if c.ly < -0.3 or c.down then
				...
				if selLine <= len(_lines) - 1 then
					sndMenuMove()
				endif
				
				selLine = min(selLine, len(_lines) - 1)
...
// Map loading message.
function loadMapMsgEd(_mapName, _objName, _objPos)
...
// Object definition loadinf message.
function loadDefMsgEd(_defName)
...
function updateMassSel()
	if g_cur.mode == "masssel" then
		if g_cur.massSel.cursorPos != g_cur.pos then
			...
			// Screen for cells; updateAllMassSelContents() will crash if no cells exist
			if len(g_cell) then
...
// Gets a list of all children in an object definition, including recursive children
// within children. Doesn't include self.
...
// Returns bank and index of an encoded bank/idx combo
...
function triggerMenuAction(_sel, _menu)
	...
	if getMenuFromChain(_menu, _sel.idxChain)[_sel.idx].active then
		sndMenuSel()
	endif
...
* Trying to place very large objects (e.g. Kat Deak/IcyRaceTrack) will cause Celqi to perform very poorly and/or freeze."
		...
		if strFind(_sel.action, "cammov") == 0 then
			if _sel.action == "cammovup" then
				g_cam.movSpd = wrapIncClamp(g_cam.movSpd + 5, 5, 1000)
			else if _sel.action == "cammovdown" then
				g_cam.movSpd = wrapIncClamp(g_cam.movSpd - 5, 5, 1000)
			else
				var newSpd = input("Camera movement speed", false)
				g_cam.movSpd = clamp(int(newSpd), 1, 1000)
			endif endif
		...
		if strFind(_sel.action, "camrot") == 0 then
			if _sel.action == "camrotup" then
				g_cam.rotSpd = wrapIncClamp(g_cam.rotSpd + 5, 5, 1000)
			else if _sel.action == "camrotdown" then
				g_cam.rotSpd = wrapIncClamp(g_cam.rotSpd - 5, 5, 1000)
			else
				var newSpd = input("Camera rotation speed", false)
				g_cam.rotSpd = clamp(int(newSpd), 1, 1000)
			endif endif
...
// Code for executing object  menu selections.
...
function closeMenu(_menu, _menuImg, _writePrefs, _dynIdxChain)
	...
	// Calling update() here prevents screen flash at low RAM level. Dunno why it happens.
	if !g_photoMode then
		drawStatusBar()
	endif
	...
	if _writePrefs then
		showLoadBox("Saving editor preferences ...", false, !g_photoMode, false)
		writeEdPrefs()
		removeLoadSpr()
	endif
...
function moveMenuVert(_menu, _dir)
	...
	if menuSel.idx > -1 then
		...
		if _dir then
			...
			if menuSel.up != menuSel.idx then
				sndMenuMove()
			endif
		else
			menuSel.menu[menuSel.down].sel = true
			
			if menuSel.down != menuSel.idx then
				sndMenuMove()
			endif
		endif
...
function openObjMenu(_objMenu)
	...
	if c.y and !g_cDat[g_cIdx.y].held then
		...
		if _objMenu.isOpen then
			...
			g_sndQueue = getCloseObjMenuAudio()
		else
			setFov(50)
			update()
			...
			drawSprites()
			drawObjects()
			g_sndQueue = getOpenObjMenuAudio()
		endif
	else if !c.y then
...
function updateObjMenu(_objMenu)
	...
	if needClose then
		// If merged, we'll already get sndMenuSel() from the merged menu
		if !len(_objMenu.obj[_objMenu.selPos.x][_objMenu.selPos.y].children) then
			sndMenuSel()
		endif
...
function applyKbHold(_keyDat, _newPress)
	...
	// Don't account for initial repeat rate, because 0.5 sec of camera 
	// motion on a short tap doesn't feel good to use.
...
function updateCamCollisions(_flush)
	...
	updateObjPhysics(g_cam.collider, colCon, colCon.scale, 
		g_cam.collisionMode, _flush, true, true, false, true, 2, true, deltaTime(), {0, 0, 0})
...
function updateCurCollisions(_flush)
	...
	var colliderPos = floorVec(g_cur.pos) + 0.5
	setObjectPos(g_cur.collider, colliderPos)
	getSimpleMapCollision(colCon, g_cur.collider, colliderPos, {0, 0, 1}, {0, 1, 0}, colCon.scale, true, _flush, 1)
...
function startThrow(_pos, _fwd, _up)
	...
	// Because we allow merged objects to be thrown even though physics can't be applied to them, we create 
	// a single invisible cube with the size of the object and use that for physics then just snap the 
	// actual object to the physics cube.
	...
	// The collider is a cube, which always has this extent. We will subsequently scale the cube to 
	// fit the base object, but that scaling will never be represented in the collider's extent itself.
	throwCon.ext = [ .hi = {1, 1, 1}, .lo = {-1, -1, -1} ]
	throwCon = initColContext(throwCon, throwCollider, _pos, g_cur.fwd, g_cur.up, colScale, 4, mass, {0, -50, 0}, 0.001, true, false, true)
	throwCon.vel = (g_cam.fwd * 15 + g_cam.up * 7)
	g_thrown = push(g_thrown, [ .obj = objContainer, .collider = throwCollider, .colCon = throwCon, .timer = 0, 
		.com = getExtCenter(objDef.ext) * g_cur.scale * resizeFactor, .delta = 0, .ticksSinceUpdate = 0, .origDelta = deltaTime() ])
return void
...
function updateThrow()
	...
	for i = 0 to len(g_thrown) loop
		if g_thrown[i].colCon.frozen < g_physicsFreezeThreshold then
			unfrozenObjs += 1
		endif
	repeat
	...
	while i < len(g_thrown) loop
		...
		if g_thrown[i].timer >= 7 or (g_photoMode and c.b) then
			...
			if g_thrown[i].colCon.frozen >= g_physicsFreezeThreshold or i - frozenOffset == g_frame % unfrozenObjs then
				if g_thrown[i].colCon.frozen >= g_physicsFreezeThreshold then
					frozenOffset += 1
				endif
				
				g_thrown[i].delta += deltaTime()
				g_thrown[i].colCon = updateObjPhysics(g_thrown[i].collider, g_thrown[i].colCon, 
					g_thrown[i].colCon.scale, g_thrown[i].colCon.collisionMode, 
					false, true, true, true, false, 1, true, g_thrown[i].delta, {0, 0, 0})
				
				// Snap the actual object, which may be merged and therefore unable to derectly use 
				// physics, to the physics object
				...
				g_thrown[i].delta = 0
				g_thrown[i].ticksSinceUpdate = 0
				frameCollisions += 1
			else
				// If an object isn't being updated this frame, estimate its position so we still get visually
				// smooth (ish) movement. Position gets corrected next time a physics update actually runs.
				g_thrown[i].ticksSinceUpdate += 1
				g_thrown[i].delta += deltaTime()
				...
				if g_thrown[i].colCon.frozen < g_physicsFreezeThreshold then
					...
					if len(g_thrown[i].colCon.objList) then
						scale -= 0.4
					endif
					
					scale -= unfrozenObjs * 0.1
					scale -= g_thrown[i].ticksSinceUpdate * 0.1
					
					velFactor = clamp(scale, 0, 1)
					
					var newVel = (g_thrown[i].colCon.vel + g_thrown[i].colCon.grav * g_thrown[i].delta) * g_thrown[i].delta * velFactor
					var newPos = g_thrown[i].colCon.comPos + newVel * (1 - g_thrown[i].ticksSinceUpdate * 0.1)
					var newFwd = axisRotVecBy(g_thrown[i].colCon.fwd, g_thrown[i].colCon.rotVelAxis, g_thrown[i].colCon.rotVel * g_thrown[i].delta)
					var newUp = axisRotVecBy(g_thrown[i].colCon.up, g_thrown[i].colCon.rotVelAxis, g_thrown[i].colCon.rotVel * g_thrown[i].delta)
					
					var appliedCom = changeVecSpaceZFwd(g_thrown[i].com, newFwd, newUp,
						safeCross(newFwd, newUp), {0, 0, 0})
					
					setObjectPos(g_thrown[i].collider, newPos)
					setObjRot(g_thrown[i].collider, newPos, newFwd, newUp)
					
					setObjectPos(g_thrown[i].obj.obj, newPos - appliedCom)
					setObjRot(g_thrown[i].obj.obj, newPos - appliedCom, newFwd, newUp)
				endif
			endif
...
function updateCam(_c)
	...
	var unconstrainedFwd = axisRotVecBy(
		newFwd,
		newR,
		_c.ry * 0.0167 * g_cam.rotSpd
	)
	var unconstrainedUp = normalize(cross(newR, unconstrainedFwd))
	
	// Prevent flipping camera upsidedown
	if unconstrainedUp.y > 0.1 then
		newFwd = unconstrainedFwd
		newUp = unconstrainedUp
	else
		newUp = normalize(cross(newR, newFwd))
	endif
	
	// Resets freeze state
	g_cam.lastColContext = setColContextVel(g_cam.lastColContext, {0, 0, 0})
	
	if _c.zr then
		if g_cam.worldMovement then
			g_cam.lastColContext.vel += flattenY(newFwd)
		else
			g_cam.lastColContext.vel += newFwd
		endif
	else if _c.zl then
		if g_cam.worldMovement then
			g_cam.lastColContext.vel += flattenY(newFwd) * -1
		else
			g_cam.lastColContext.vel += newFwd * -1
		endif
	endif endif
	
	loop if _c.up then
		if g_cam.worldMovement then
			g_cam.lastColContext.vel += {0, 1, 0}
		else
			g_cam.lastColContext.vel += newUp
		endif
		break endif
	if _c.down then
		if g_cam.worldMovement then
			...
			g_cam.lastColContext.vel += newUp * -1
		endif
		break endif
	if _c.right then
		g_cam.lastColContext.vel += newR
		break endif
	if _c.left then
		g_cam.lastColContext.vel += newR * -1
		break
	endif break repeat
	
	g_cam.lastColContext.vel *= 0.167 * g_cam.movSpd
	var rotDeg = getAngleBetweenVecs(oldFwd, newFwd)
	var rotAxis = safeCross(oldFwd, newFwd)
	
	if rotAxis == {0, 0, 0} then
		rotAxis = newUp
	endif
	
	g_cam.lastColContext.rotVel = rotDeg * 100
	g_cam.lastColContext.rotVelAxis = rotAxis
	
	if g_cam.collisionMode then
		updateCamCollisions(false)
	else
		g_cam.lastColContext.fwd = getNewColFwd(g_cam.lastColContext, deltaTime())
		g_cam.fwd = g_cam.lastColContext.fwd
		g_cam.r = safeCross(g_cam.fwd, {0, 1, 0})
		
		if g_cam.r == {0, 0, 0} then
			g_cam.lastColContext.up = getNewColUp(g_cam.lastColContext, deltaTime())
		else
			g_cam.lastColContext.up = axisRotVecBy(g_cam.r, g_cam.fwd, -90)
		endif
		g_cam.up = g_cam.lastColContext.up
		g_cam.pos += g_cam.lastColContext.vel * deltaTime()
...
function advanceCurMode(_c)
	...
	if _c.r then
		if !g_cDat[g_cIdx.r].held and g_cur.mode != "masssel" then
			...
			g_sndQueue = getCycleWidgetAudio()
		endif
	else if !_c.r then
		g_cDat[g_cIdx.r].held = false
	endif endif	
...
// Sound for menu move.
function sndMenuMove()
	var clipThresh = 0.1
	var clipStr = 6
	setClipper(g_chMenu, clipThresh, 3)
	setClipper(g_chMenu + 1, clipThresh, 3)
	
	setReverb(g_chMenu, 0, 0)
	setReverb(g_chMenu + 1, 0, 0)
	
	setFilter(g_chMenu, 2, 800)
	setFilter(g_chMenu + 1, 2, 2000)
	
	if g_cDat[g_cIdx.lxy].count > 1 then
		playNote(g_chMenu, 4, note2Freq(110), 0.2, 880, 0.5)
		playNote(g_chMenu + 1, 3, note2Freq(84), 0.5, 290, 0.5)
	else
		playNote(g_chMenu, 4, note2Freq(110), 0.6, 880, 0.5)
		playNote(g_chMenu + 1, 3, note2Freq(77), 6, 290, 0.5)
	endif
return void

// Sound for widget change.
function sndCursor()
	var clipThresh = 0.1
	var clipStr = 6
	setClipper(g_chCursor, 999, 0.01)
	setClipper(g_chCursor + 1, 999, 0.01)
	
	setReverb(g_chCursor, 0, 0)
	setReverb(g_chCursor + 1, 0, 0)
	
	setFilter(g_chCursor, 2, 1000)
	setFilter(g_chCursor + 1, 2, 2000)
	
	if g_cDat[g_cIdx.lxy].count > 1 then
		playNote(g_chCursor, 4, note2Freq(110), 0.07, 880, 0.5)
		playNote(g_chCursor + 1, 1, note2Freq(96), 0.02, 800, 0.5)
	else
		playNote(g_chCursor, 4, note2Freq(110), 0.07, 880, 0.5)
		playNote(g_chCursor + 1, 1, note2Freq(84), 0.02, 290, 0.5)
	endif
return void

// Sound for menu selection.
function sndMenuSel()
	var clipThresh = 0.1
	var clipStr = 6
	setClipper(g_chMenu, clipThresh, 3)
...
function getSplashAudio()
	var audioQueue = initQueue("SplashAudio", time() + 0.7, [ [ .beatPos = {0, 0}, .bpm = 140, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.2
	var spd = 150
	var pan = 0.5
	...
	audioQueue.queue = [
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch, .arg = [30, 0.5] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch + 1, .arg = [25, 0.6] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch + 2, .arg = [25, 0.6] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch, .arg = [1, 3500] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 1, .arg = [1, 1000] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 2, .arg = [1, 1000] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 3, .arg = [1, 600] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 4, .arg = [1, 4000] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 5, .arg = [1, 600] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 6, .arg = [1, 4000] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(60), vol * 1.1, spd * 0.8, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(55), vol2 * 1.1, spd2 * 0.8, pan2] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 2, .arg = [wav2, note2Freq(48), vol2 * 1.1, spd2 * 0.8, pan3] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 3, .arg = [3, note2Freq(36), vol * 1.1, 5, 0] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 4, .arg = [2, note2Freq(79), vol * 0.05, 5, 0] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 5, .arg = [2, note2Freq(36), vol * 0.05, 5, 1] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 6, .arg = [3, note2Freq(79), vol * 1, 5, 1] ],
		
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(67), vol * 0.7, spd * 1.4, pan] ],
		
		[ .beatPos = {0, 0.5}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol, spd * 0.8, pan] ],
		[ .beatPos = {0, 0.5}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(55), vol2 * 1.1, spd2 * 0.8, pan2] ],
		[ .beatPos = {0, 0.5}, .timePos = 0, .func = "playNote", .ch = ch + 2, .arg = [wav2, note2Freq(48), vol2 * 1.1, spd2 * 0.8, pan3] ],
		
		[ .beatPos = {0, 0.75}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(58), vol * 0.5, spd * 1.6, pan] ],
		
		[ .beatPos = {0, 1}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(62), vol * 1.1, spd * 0.8, pan] ],
		
		[ .beatPos = {0, 1.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(67), vol * 0.7, spd * 1.4, pan] ],
		
		[ .beatPos = {0, 1.5}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol * 1.1, spd * 0.8, pan] ],
		[ .beatPos = {0, 1.5}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(53), vol2 * 1.1, spd2 * 0.8, pan2] ],
		[ .beatPos = {0, 1.5}, .timePos = 0, .func = "playNote", .ch = ch + 2, .arg = [wav2, note2Freq(50), vol2 * 1.1, spd2 * 0.8, pan3] ],
		
		[ .beatPos = {0, 1.75}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(58), vol * 0.6, spd * 1.6, pan] ],
		
		[ .beatPos = {0, 2}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(60), vol, spd * 0.7, pan] ],
		[ .beatPos = {0, 2}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(55), vol2 * 1.1, spd2 * 0.8, pan2] ],
		[ .beatPos = {0, 2}, .timePos = 0, .func = "playNote", .ch = ch + 2, .arg = [wav2, note2Freq(48), vol2 * 1.1, spd2 * 0.8, pan3] ],
		
		[ .beatPos = {0, 3}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(84), vol * 0.4, spd * 0.6, pan] ],
		[ .beatPos = {0, 3}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [3, note2Freq(79), vol * 0.3, spd * 0.5, pan] ]
	]
return audioQueue

// Sound for loaded file.
function getLoadedAudio()
	var audioQueue = initQueue("LoadedAudio", time() + 0.2, [ [ .beatPos = {0, 0}, .bpm = 220, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.2
	var spd = 150
	var pan = 0.5
	...
	audioQueue.queue = [
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch, .arg = [0, 0] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch + 1, .arg = [0, 0] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setReverb", .ch = ch + 2, .arg = [0, 0] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch, .arg = [1, 3500] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 1, .arg = [1, 1000] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 2, .arg = [1, 1000] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(60), vol, spd * 0.7, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(55), vol2 * 1.1, spd2 * 0.8, pan2] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 2, .arg = [wav2, note2Freq(48), vol2 * 1.1, spd2 * 0.8, pan3] ],
		
		[ .beatPos = {0, 0.5}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(84), vol * 0.4, spd * 0.6, pan] ],
		[ .beatPos = {0, 0.5}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [3, note2Freq(79), vol * 0.3, spd * 0.5, pan] ]
	]
return audioQueue

// Sound for menu open.
function getOpenMenuAudio()
	var audioQueue = initQueue("OpenMenuAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 220, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.15
	var spd = 200
	var pan = 0.5
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch, .arg = [1, 4500] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setFilter", .ch = ch + 1, .arg = [1, 1000] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setClipper", .ch = ch, .arg = [999, 0.01] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "setClipper", .ch = ch + 1, .arg = [999, 0.01] ],
		
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(60), vol, spd, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(57), vol2, spd2, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(65), vol, spd, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(60), vol2, spd2, pan] ]
	]
return audioQueue

// Sound for menu close.
function getCloseMenuAudio()
	var audioQueue = initQueue("OpenMenuAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 220, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.16
	var spd = 200
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(65), vol, spd, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(60), vol2, spd2, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(60), vol, spd, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(57), vol2, spd2, pan] ]
	]
return audioQueue

// Sound for object menu open.
function getOpenObjMenuAudio()
	var audioQueue = initQueue("OpenMenuAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 220, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.15
	var spd = 200
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol, spd, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(69), vol2, spd2, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(77), vol, spd, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(72), vol2, spd2, pan] ]
	]
return audioQueue

// Sound for object menu close.
function getCloseObjMenuAudio()
	var audioQueue = initQueue("OpenMenuAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 220, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.15
	var spd = 200
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(77), vol, spd, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(72), vol2, spd2, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol, spd, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(69), vol2, spd2, pan] ]
	]
return audioQueue

// Sound for cycling widget.
function getCycleWidgetAudio()
	var audioQueue = initQueue("CycleWidgetAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 330, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.15
	var spd = 200
	var pan = 0.5
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol, spd, pan] ],
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(67), vol2, spd2, pan] ],
		[ .beatPos = {0, 0.35}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(72), vol, spd, pan] ],
		[ .beatPos = {0, 0.25}, .timePos = 0, .func = "playNote", .ch = ch + 1, .arg = [wav2, note2Freq(65), vol2, spd2, pan] ]
	]
return audioQueue

// Sound for cycling widget mode.
function getCycleModeAudio()
	var audioQueue = initQueue("CycleWidgetAudio", time(), [ [ .beatPos = {0, 0}, .bpm = 330, .timeSig = 4 ] ])
	var wav = 0
	var vol = 0.15
	var spd = 200
	...
	audioQueue.queue = [
		...
		[ .beatPos = {0, 0}, .timePos = 0, .func = "playNote", .ch = ch, .arg = [wav, note2Freq(79), vol, spd, pan] ],
